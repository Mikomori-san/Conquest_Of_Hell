...
private:
	...
	std::shared_ptr<BossAbility> m_ability1 = std::make_shared<MeleeBA>(MeleeBA("MeleeBA", gameObject, m_playerPtr));
	std::shared_ptr<BossAbility> m_ability2 = std::make_shared<CharmBA>(CharmBA("CharmBA", m_playerPtr));
	std::shared_ptr<CharmIndicator> m_charmInd = std::make_shared<CharmIndicator>(CharmIndicator(gameObject));
